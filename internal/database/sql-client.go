...
func (s *MySql) UpdateOrderItems(orderId int64, items []OrderProductData, currencyValue float64, orderTotal float64) error {
	...
	insertQuery := fmt.Sprintf(`
		INSERT INTO %sorder_product (order_id, product_id, product_uid, name, model, quantity, price, total, tax, reward, sku, upc, ean, jan, isbn, mpn, location, weight)
		SELECT ?, p.product_id, p.product_uid, pd.name, p.model, ?, ?, ?, ?, 0, p.sku, p.upc, p.ean, p.jan, p.isbn, p.mpn, p.location, p.weight
		FROM %sproduct p
